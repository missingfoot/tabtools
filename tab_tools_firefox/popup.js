...
async function restoreSession(session) {
  try {
    ...
    // Firefox's session restore) until the user clicks them.
...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  document.getElementById('openUrls').addEventListener('click', async function() {
    try {
      ...
      // sit unloaded (like Firefox's session restore) until the user clicks them.
  ...
  document.getElementById('openUrlsInNewWindow').addEventListener('click', async function() {
    try {
      ...
      // (like Firefox's session restore) until the user clicks them.
      const [firstUrl, ...restUrls] = validUrls;
      const newWindow = await chrome.windows.create({ url: firstUrl });
      await Promise.all(restUrls.map(async (url) => {
        const tab = await chrome.tabs.create({ windowId: newWindow.id, url, active: false });
